...
import com.dragon.facedetectorflow.program.ProgramPool
import com.dragon.facedetectorflow.texture.DoubleFrameBufferTexture
import com.dragon.facedetectorflow.texture.FrameBufferTexture
...
class NodesRender(
    ...
    var haveFace = false
    val facePoints = FloatArray(84*2)
    ...
    fun onTouch(action: Int, x: Float, y: Float): Boolean {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            targetNode = nodes.asSequence().filter { node -> node.contain(x, y) && node.onTouch(this, action, x, y) }.firstOrNull()
        }
        return targetNode?.onTouch(this, action, x, y) ?: false
    }
