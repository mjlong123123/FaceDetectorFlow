package com.dragon.facedetectorflow.utils

import android.opengl.Matrix

class MVPMatrix : Cloneable {
    companion object {
    ...
    fun scale(scaleX: Float, scaleY: Float, centerX: Float, centerY: Float): MVPMatrix {
        Matrix.setIdentityM(values, M_INDEX)
        Matrix.translateM(values, M_INDEX, centerX, centerY, 0f)
        Matrix.scaleM(values, M_INDEX, scaleX, scaleY, 1f)
        Matrix.translateM(values, M_INDEX, -centerX, -centerY, 0f)
        Matrix.multiplyMM(values, MVP_INDEX, values, VP_INDEX, values, M_INDEX)
        return this
    ...
    public override fun clone(): MVPMatrix {
        return MVPMatrix().also {
            it.viewPortHeight = viewPortHeight
            it.viewPortWidth = viewPortWidth
            System.arraycopy(values, 0, it.values, 0, values.size)
        }
    }
